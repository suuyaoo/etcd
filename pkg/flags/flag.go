...
func verifyEnv(lg *zap.Logger, prefix string, usedEnvKey, alreadySet map[string]bool) {
	for _, env := range os.Environ() {
		...
		if len(kv) != 2 {
			if lg != nil {
				lg.Warn("found invalid env", zap.String("env", env))
			}
		}
		if usedEnvKey[kv[0]] {
			continue
		...
		if alreadySet[kv[0]] {
			if lg != nil {
				lg.Fatal("conflicting environment variable is shadowed by corresponding command-line flag (either unset environment variable or disable flag)", zap.String("key", kv[0]))
			}
		}
		if strings.HasPrefix(env, prefix+"_") {
			if lg != nil {
				lg.Warn("unrecognized environment variable", zap.String("env", env))
			}
		}
	}
}
...
func setFlagFromEnv(lg *zap.Logger, fs flagSetter, prefix, fname string, usedEnvKey, alreadySet map[string]bool, log bool) error {
	...
	if !alreadySet[key] {
		...
		if val != "" {
			...
			if log && lg != nil {
				lg.Info("recognized and used environment variable", zap.String("key", key), zap.String("val", val))
			}
		}
	}
	return nil
